...
import {
	DROP_DOWN_FLOWER,
	DROP_DOWN_FOLIAGE, DROP_DOWN_SUCCULENT,
	DROP_DOWN_VALUE_V2,
	DROP_DOWN_VALUE_V2_FRUIT
} from "../../../../constants/drop-down-values";
import { REGIONS } from '../../../../constants/region-values';

@Component({
  selector: 'app-create-listing-decorative',
	imports: [
		AsyncPipe,
		DropDownComponent,
		FormsModule,
  ...
  templateUrl: './create-listing-decorative.component.html',
  styleUrl: './create-listing-decorative.component.less'
})
export class CreateListingDecorativeComponent {
		...
		if (this.createListingForm.valid) {
			...
			this._listingService.createListing(listingModel)
				...
				.subscribe((response: boolean) => {
					if (response) {
						dialogRef.close();
						this._router.navigate(["/page2"], { state: {data: Type.DECORATIVE}, skipLocationChange: true });
					}
				});
		} else {
	...
	selectCropType(value: string) {
		...
		if (value === '1') {
			this.DROP_DOWN_VALUE$.next(this.DROP_DOWN_FOLIAGE);
			...
			this.DROP_DOWN_VALUE$.next(this.DROP_DOWN_FLOWER);
			cropName?.clearValidators();
			cropName?.updateValueAndValidity();
			...
			this.DROP_DOWN_VALUE$.next(this.DROP_DOWN_SUCCULENT);
	...
	protected readonly DROP_DOWN_FOLIAGE = DROP_DOWN_FOLIAGE;
	protected readonly DROP_DOWN_FLOWER = DROP_DOWN_FLOWER;
	protected readonly DROP_DOWN_SUCCULENT = DROP_DOWN_SUCCULENT;
	protected DROP_DOWN_VALUE$ = new BehaviorSubject<DropDownValue[]>(this.DROP_DOWN_FOLIAGE);
	protected readonly REGIONS = REGIONS;
	protected readonly CROP_SELECTION = [{ label: 'Foliage', value: '1' }, { label: 'Flowering', value: '2' },{ label: 'Succulents', value: '3' }, { label: 'Other', value: '4' }];
}
